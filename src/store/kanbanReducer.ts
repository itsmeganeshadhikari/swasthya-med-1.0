...
const kanbanReducer = (state = initialState, action: KanbanPayloadProps) => {
  let newColumn: KanbanColumn[];
  let currentColumn: KanbanColumn;
  let newUserStory: KanbanUserStory[];
  let currentStory: KanbanUserStory;
  let newItems: KanbanItem[];
  let itemIds: string[];
  ...
  switch (action.type) {
    ...
    case ADD_ITEM:
      if (action.payload.columnId !== "0") {
        newColumn = state.columns.map((column: KanbanColumn) => {
      ...
      if (action.payload.storyId !== "0") {
        newUserStory = state.userStory.map((story: KanbanUserStory) => {
    ...
    case ADD_ITEM_COMMENT:
      newItems = state.items.map((item: KanbanItem) => {
    ...
    case DELETE_ITEM:
      newColumn = state.columns.map((column: KanbanColumn) => {
        itemIds = column.itemIds.filter(
          (id: string) => id !== action.payload.itemId
        );
        return {
          ...column,
          itemIds,
